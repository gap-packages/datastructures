...
#
# For documentation of the data structure, see Wikipedia. A skip list is essentially set of linked lists called
# levels -- in this implementation levels currently start at 2. The list at level 2 contains all the objects in the 
# set in order, the lists at higher levels contain progressively fewer of them (roughly 1/3 as many as at the level below).
# each level is a subset of the one below. The nodes of the lists are the same objects, so that you can drop down from one list 
# to a lower level as you approach the object you want.
#
#
#
# Each node is represented by a plain list whose 1 entry is the value at the node and whose other entries
# are next pointers for all of the linked lists that that entry is in.  The end of the lists is 
# represented by unbound. 
#
# The root object contains the head pointers of all the lists (and fail in position 1)
# This object (along with the less than function and other attributes are stored in an outer component object
#


#
# Recorde for local functions and constants to avoid cluttering the global namespace
#
SKIPLISTS := rec();
...
# the default value is 1/3 represented by its inverse here. It is possible that 1/2 might be better choice when comparison 
# is very expensive.
#
...
# immediately precedes the value we are looking for (or would precede it if it were present). 
# So if it returns x with an entry in position l
# then x[l][1] < val (or x[1] is the head node) and x[l][l] is either unbound or has x[l][l][1] >= val
#

#
# There is a C version of this function in skiplist.c. This version is retained (and tested) as a reference
# implementation and/or if the kernel code is not compiled. 
#
