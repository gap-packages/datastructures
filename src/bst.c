...
Obj DS_BST_FIND(Obj self, Obj bst, Obj val, Obj less) {
  ...
  while (1) {
    ...
    GAP_ASSERT(IS_PLIST(child) && LEN_PLIST(child) >= BST_VAL && ELM_PLIST(child,BST_VAL));
...
static Obj avli2(Obj avl, UInt dirn, Obj val, Obj less, Obj trinode) {
  Obj child;
  UInt i = (dirn == 1) ? BST_RIGHT : BST_LEFT;
  UInt x;
  Obj deeper;
  if (!(child = ELM_PLIST(avl,i))) {
	...
	return CALL_2ARGS(trinode, avl, INTOBJ_INT(dirn));
      }
    } else {
...
static Obj DS_AVL_ADDSET_INNER(Obj self, Obj avl, Obj val, Obj less, Obj trinode)
{
  ...
  return EQ(val, d) ? Fail:
    avli2(avl,
	  (True == CALL_2ARGS(less, val, d)) ? -1 : 1,
	  val, less, trinode);    
}
			     
static Obj DS_AVL_REMSET_INNER(Obj self, Obj node, Obj val, Obj less, Obj remove_extremal, Obj trinode, Obj remove_this) {
  ...
  if (!child)
    return Fail;
  ret = DS_AVL_REMSET_INNER(0, child, val, less, remove_extremal, trinode, remove_this);
  if (ret == Fail)
    return Fail;
  ...
  if (ELM_PLIST(ret,1) == INTOBJ_INT(0)) {
    out = NEW_PLIST(T_PLIST_DENSE, 2);
    SET_LEN_PLIST(out,2);
    SET_ELM_PLIST(out,1,INTOBJ_INT(0));
    SET_ELM_PLIST(out,2,node);
    CHANGED_BAG(out);
    return out;
  }
  ...
  if ((im == -1 && i == BST_LEFT) || (im == 1 && i == BST_RIGHT)) {
    ...
    SET_ELM_PLIST(node, AVL_IMBALANCE, INTOBJ_INT((i == BST_LEFT) ? 1 : -1));
    out = NEW_PLIST(T_PLIST_DENSE, 2);
    SET_LEN_PLIST(out,2);
    SET_ELM_PLIST(out,1,INTOBJ_INT(0));
    SET_ELM_PLIST(out,2,node);
...
static StructGVarFunc GVarFuncs[] = {
  ...
  GVARFUNC(DS_AVL_REMSET_INNER, 6, "avl, val, lessFunc, remove_extremal, trinode, remove_this"),
  { 0 }
};
